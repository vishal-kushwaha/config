...
if [[ $MODE == "dark" ]]; then
	...
	cp -f "$LAZYGIT_CONFIG_PATH/../themes/lazygit/themes/mocha/rosewater.yml" "$LAZYGIT_CONFIG_PATH"
else
	sed -i "" "s/_mocha/_latte/g" "$ALACRITTY_CONFIG_PATH"
	sed -i "" "s/mocha/latte/g" "$TMUX_CONFIG_PATH"
	cp -f "$LAZYGIT_CONFIG_PATH/../themes/lazygit/themes/latte/rosewater.yml" "$LAZYGIT_CONFIG_PATH"
fi
